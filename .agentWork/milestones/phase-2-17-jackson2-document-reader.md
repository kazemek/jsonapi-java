...
> **Dependencies:** Phases 2.4, 2.11, 2.12, and 2.16  
> **Status:** Not started
...
- Phase 2.4 defines the stable read contract: explicit resource versus resource-identifier
...
- Phase 2.12 owns capability-tagged positive and read-only negative fixtures with stable category,
  pointer, and rule-code expectations.
...
- Add Jackson 2 reader entry points under `io.github.kazemek.jsonapi.jackson2` that consume common
  read contexts/diagnostics and preserve Phase 2.4 ownership and primary-data-kind rules.
...
- Run the Phase 2.12 positive, ambiguous, extension/profile, malformed, local-validation, and
...
- Duplicating common contexts/diagnostics or forking the Phase 2.12 fixture corpus.
...
- Unknown/additional member classification, open-value conversion, constructor use, and aggregate
  validation follow Phase 2.4 exactly.
...
- Production code imports no `tools.jackson.*`, `jsonapi-java-jackson3`, or `core.internal` types;
  shared behavior is proven by Phase 2.12 fixtures rather than cross-major runtime delegation.

## Test strategy

- Parameterize Phase 2.12 read/round-trip fixtures and assert equal `JsonApiDocument` values from
...
- [ ] All applicable Phase 2.12 read fixtures produce model values equal to Jackson 3 results,
