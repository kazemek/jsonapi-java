...
- **Order (complete nesting):** iterate primary resources in input order as the *outer* loop; for each primary resource, iterate requested `IncludePath`s in caller order as the *inner* loop; within each path, walk relationship targets level-by-level (BFS along that path), iterating to-many target collections in their collection order. Global `included` order is first-discovery across this walk. First discovery also determines the canonical representation of an identity: a later `ResourceObject` for an already-seen identity that differs fails `CONFLICTING_INCLUDED_REPRESENTATION`.
...
- Spock `CompoundSerializationSpec` covering cyclic, shared, conflicting, empty, nested, multi-primary + multi-path, malformed-path, and limit-exceeding graphs with explicit include paths; reuse `Article`/`Comment`/`Person` and add access-counting (traversal-scoped), conflicting-representation, and deep-nested test models; assert produced documents pass `JsonApiDocumentWriter` validation (full linkage) and `included: []` on empty resolution. Add the two canonical compound fixtures (nested-intermediate, shared-identity) with manifest entries to `fixtures/jsonapi-1.1/` (see Test strategy).
...
- Limits: request a path longer than `maxDepth` and a graph that exceeds `maxIncluded`; assert the exact diagnostic code and dotted JSON:API `propertyPath`.
- Invalid paths: segment naming a non-relationship and a denied relationship; malformed `IncludePath.of(String)` input (`""`, `.a`, `a.`, `a..b`, whitespace-only); assert `INVALID_INCLUDE_PATH` / `DENIED_RELATIONSHIP_INCLUDE`.
