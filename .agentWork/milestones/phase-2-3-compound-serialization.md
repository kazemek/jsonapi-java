...
- **`IncludePath.of(String)` syntax:** a valid path is one or more dot-separated non-empty relationship names. Reject — with `INVALID_INCLUDE_PATH` — empty input, leading or trailing dots, repeated dots, and whitespace-only segments; do not silently trim or otherwise normalize input. Preserve the ordered relationship-name semantics.
...
- **Output deduplication (separate from traversal):** seed an included-identity tracker with every primary `data` `ResourceIdentity` so a primary resource is never emitted in `included` (self-links and primary-to-primary relationship targets remain linkage-only). Deduplicate `included` entries by `ResourceIdentity` (type+id or type+lid via `ResourceObject.identityKey()`). Reject a later unequal `ResourceObject` for an already-seen included identity with `CONFLICTING_INCLUDED_REPRESENTATION`. First discovery determines the canonical representation and `included` order.
- **Traversal visit state (separate from output dedup):** track visits by `(ResourceIdentity, include-path position)` (or equivalent). Discovering an identity for path prefix `a` must not prevent continuing along a longer requested path such as `a.b`, and converging paths that share an intermediate must still traverse explicitly requested suffixes. Do **not** treat “already present in the included-identity set” as “stop walking this path.”
...
- Extend `JsonApiResourceMapper` with unambiguous three-argument overloads only (do **not** add two-argument context overloads that collide with `@Nullable DocumentEnvelope`):
  ...
  Callers without an envelope pass `null` for the envelope argument (`toDocument(resource, null, context)`). Add internal `CompoundInclusionEngine` that pre-validates paths (precedence above), walks the graph for inclusion via selective relationship reads with `(identity, pathIndex)` visit tracking, builds primary and included resources with `DomainResourceWriter.toResource` (full linkage) and uses `extractIdentifier` where linkage-only identity is needed, seeds output dedup with primary identities, and produces an ordered `List<ResourceObject>` for `JsonApiDocument.included`. Update `buildDocument` to accept `@Nullable List<ResourceObject> included` (`null` = omit the member; empty list = emit `included: []`).
...
- Prefix-overlapping and converging paths: paths such as `a` and `a.b` (and paths that meet on one intermediate) traverse requested suffixes; first-discovery order and conflict checks still hold.
...
- Invalid paths: segment naming a non-relationship and a denied relationship; malformed `IncludePath.of(String)` input (`""`, `.a`, `a.`, `a..b`, whitespace-only); multi-failure request asserting first-win precedence (depth → mapping → policy per path order); assert `INVALID_INCLUDE_PATH` / `DENIED_RELATIONSHIP_INCLUDE` / `INCLUDE_DEPTH_EXCEEDED`.
...
- Canonical fixtures: add two richer compound fixtures — one nested-intermediate (`comments.author`) and one shared-identity — to `fixtures/jsonapi-1.1/` with manifest entries, so Phase 2.5's draft-schema cross-check and Phase 2.13's Jackson 2 parity reuse shared bytes rather than major-specific test models. The existing single-level `compound-document.json` is retained.
