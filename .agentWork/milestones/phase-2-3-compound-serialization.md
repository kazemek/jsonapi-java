...
- **`included` null vs empty contract:** context-free overloads, a `null` context, and a context whose include-path set is empty or `null` all mean *no inclusion request* — `JsonApiDocument.included` is `null` (member omitted). A context with at least one non-empty valid path that resolves to zero resources is an *inclusion request with an empty result* — `included: []` is emitted. The distinction is "did not ask" (omit) versus "asked but got nothing" (emit `[]`).
...
- The engine produces `List<ResourceObject>` (or `null` when no inclusion request was supplied — context-free overload, `null` context, or a context with zero/empty paths); `JsonApiDocument` is constructed via `buildDocument` with the populated `included` list when includes were requested (`null` omits the member; empty list emits `included: []`). The writer's aggregate validation runs unchanged; no `sparseFieldsetException` is set by this milestone.
...
- [ ] Generated compound documents pass `JsonApiDocumentWriter` identity/full-linkage/lid validation; `included: []` is emitted for a context with at least one non-empty path resolving to no resources; context-free overloads, `null` contexts, and contexts with zero/empty paths all omit `included`.
