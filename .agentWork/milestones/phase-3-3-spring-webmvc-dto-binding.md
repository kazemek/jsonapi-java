...
- Request arguments bind via `JsonApiDomainDocumentReader.fromDocument` on the already-validated
  `JsonApiDocument` from Phase 3.2 transport (registry bean configures `domainDocumentReader`).
  Do not re-parse the body with `readValue` after the document converter.
- Presence-aware PATCH controller arguments are Phase 3.4.
...
- Presence-aware PATCH command arguments (Phase 3.4).
...
- WebFlux support; Phase 3.5 evaluates the stable WebMVC contract separately.
...
- Request resolution calls `fromDocument` on the Phase 3.2-validated `JsonApiDocument`; it does not
