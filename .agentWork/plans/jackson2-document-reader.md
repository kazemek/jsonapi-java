...
- [Jackson 2 Document Writer](jackson2-document-writer.md) establishes the Jackson 2 artifact, supported `2.22.x` / BOM `2.22.1` baseline, and
...
- Automatic domain hydration, included-resource wiring, persistence, or PATCH execution;
  [Jackson 2 Flat DTO Reader](jackson2-flat-dto-reader.md),
  [Jackson 2 Typed Domain Envelope](jackson2-typed-domain-envelope.md), and
  [Jackson 2 Presence-Aware PATCH Binding](jackson2-presence-aware-patch-binding.md) compose this
  validated reader into narrower DTO/envelope/command contracts.
- Reflection-based runtime selection between Jackson major versions.
- Input/resource limits, which remain [Conformance and Hardening](conformance-and-hardening.md) work.
...
- Run dependency/ArchUnit allowlist checks for Jackson 2 reader production code. [Jackson 2 Document Writer](jackson2-document-writer.md) owns
...
- [ ] The canonical `module-docs` checklist and conformance notes cover the completed Jackson 2
      read flow without claiming domain hydration or [Conformance and Hardening](conformance-and-hardening.md) limits.
