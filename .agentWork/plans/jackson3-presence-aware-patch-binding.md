# Jackson 3 Presence-Aware PATCH Binding
...
- [JSON:API 1.1 updating resources](https://jsonapi.org/format/1.1/#crud-updating) and
  ...
  plan binds only supplied mapped members.
...
- `jsonapi-java-jackson-common` (`io.github.kazemek.jsonapi.jackson`) holds Jackson-import-free
  ...
  `jsonapi-java-jackson3`; this plan amends it (jackson-common move policy) so
...
- Initial shared PATCH scenario inventory (stable ids for [Jackson 2 Presence-Aware PATCH Binding](jackson2-presence-aware-patch-binding.md) reuse; Jackson-major-neutral
...
- The plan lands as one reviewable commit: the neutral command contracts, the ADR-007
...
- [ ] The initial shared PATCH scenario inventory is cataloged as `PatchScenarios` exposing the
      ...
      (`custom deserializer applies to attribute change`, `patch-custom-linkage-conversion`,
      `explicit null on Optional attribute stores value == null`, `fromDocument missing id`)
      for [Jackson 2 Presence-Aware PATCH Binding](jackson2-presence-aware-patch-binding.md) parity, with no shared exclusion manifest.
