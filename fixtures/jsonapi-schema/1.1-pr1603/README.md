...
`sha256.sum` repeats the hashes in machine-readable form; the test harness
(`JsonApiDraftSchemaSpec`) fails if any vendored file drifts from its recorded hash.
...
the allow-listed fixture results in `JsonApiDraftSchemaSpec`. Follow the upstream issue
