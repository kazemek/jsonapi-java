...
public final class Attributes {
  ...
  private static Map<String, Object> copyMembers(Map<String, ?> source, boolean allowAt) {
    ...
    for (Map.Entry<String, ?> entry : source.entrySet()) {
      ...
      validateAttributeName(name, allowAt);
      copy.put(name, OpenJsonValues.copy(entry.getValue(), PATH + "/" + name));
    }
    return OrderedMaps.copyOfNullableValues(copy);
  }

  private static void validateAttributeName(String name, boolean allowAt) {
    if (MemberNames.isAtMember(name)) {
      if (!allowAt || !MemberNames.isValid(name)) {
