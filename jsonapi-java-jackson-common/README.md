...
`StructuredPatch`, `StructuredMember`, `StructuredMemberState`). `PatchChange` sealed variants cover
resource-meta and relationship-meta changes per
[ADR-015](../docs/adr/015-flat-whole-object-meta-mapping.md); identifier meta is not a variant and
rides on `ResourceIdentifier` values inside `RelationshipChange` per
