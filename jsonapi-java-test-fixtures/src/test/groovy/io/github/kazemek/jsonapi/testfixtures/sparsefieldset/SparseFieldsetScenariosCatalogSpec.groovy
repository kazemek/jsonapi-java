...
class SparseFieldsetScenariosCatalogSpec extends Specification {
  ...
  def "catalog ids are unique"() {
    expect:
    SparseFieldsetScenarios.all()*.id.toSet().size() == SparseFieldsetScenarios.all().size()
  }

  def "byId returns each registered scenario"() {
    expect:
    SparseFieldsetScenarios.all().every { SparseFieldsetScenarios.byId(it.id).is(it) }
  }

  def "catalog uses each operation and request variant"() {
    expect:
    def operations = SparseFieldsetScenarios.all()*.operation() as Set
    assert operations.containsAll(EnumSet.allOf(SparseFieldsetOperation))
    def requests = SparseFieldsetScenarios.all()*.request()
  ...
  def "success entries include mapped, unmapped, concurrent, and identity outcomes"() {
    expect:
    def expectations = SparseFieldsetScenarios.all()*.expectation()
  ...
  def "mapped successes pin absent included distinctly from a present list"() {
    expect:
    def successes = SparseFieldsetScenarios.all()
  ...
  def "failure expectations pin a non-blank property path when present"() {
    expect:
    SparseFieldsetScenarios.all().each { scenario ->
  ...
  def "access-counting success carries the shared zero-read guarantee"() {
    given:
    def scenario = SparseFieldsetScenarios.byId("access counting proves linkage vs traversal split")
  ...
  def "where with a matching predicate returns the full catalog and a rejecting predicate is empty"() {
    ...
    SparseFieldsetScenarios.where({ false }).isEmpty()
  }
