...
`sha256.sum` repeats the hashes in machine-readable form; `PinnedJsonApiSchemaResourcesSpec` in
`jsonapi-java-test-support` fails if any vendored file drifts from its recorded hash. Adapter
suites consume the same classpath resources for output-versus-schema execution.
...
the allow-listed fixture results in adapter `JsonApiDraftSchemaSpec` suites. Follow the upstream issue
