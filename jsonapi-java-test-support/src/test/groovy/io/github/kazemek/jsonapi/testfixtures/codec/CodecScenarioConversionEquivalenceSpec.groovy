...
class CodecScenarioConversionEquivalenceSpec extends Specification {
  ...
  def "ambiguous catalog order, notes, and toString match the Groovy sources"() {
    ...
    expect:
    AmbiguousPrimaryDataScenarios.all()*.id == expected*.id
    AmbiguousPrimaryDataScenarios.all().eachWithIndex { fixture, i ->
